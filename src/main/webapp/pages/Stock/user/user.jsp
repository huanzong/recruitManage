...
<script type="text/javascript">
    $(function () {
        cols = [
            ...
            {field: 'question', title: '密码问题', width: 80, sortable: true},
            {field: 'answer', title: '密码答案', width: 80, sortable: true},
            {field: 'statusname', title: '状态', width: 80, sortable: true},
            {field: 'typename', title: '类型', width: 80, sortable: true},
            {
                field: 'opt', title: '操作', width: 80, align: 'center',
                formatter: function (value, rec) {
                    var t = "禁用";
                    if (rec.status == 1) t = "启用";
                    var btn = "<a class='easyui-linkbutton' iconCls='icon-search' href='javascript:;' onclick='ban(" + rec.id + "," + rec.status + ")'>" + t + "</a>";
                    return btn;
                }
            }
        ];
        datagridInit("/userReview/index", cols);
    });

            function openAdd() {
                openDialog("新增", "userEdit.jsp", 500, 300, add);
            }

            function openEdit() {
                openDialog("修改", "userEdit.jsp", 500, 300, edit, $('#tt').datagrid('getSelected'));
            }
            ...
            function ban(id, status) {
                ...
                $.messager.confirm('修改状态', '确定' + s + '？', function (r) {
                    if (r) {
                        $.getJSON("/userReview/ban", {id: id}, function () {
                            reload();
                        });
                    }
                });
