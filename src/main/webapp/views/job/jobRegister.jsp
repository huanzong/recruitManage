...
// 	});
            $.formValidator.initConfig({
                formID: "jobform", debug: false, submitOnce: true,
                onError: function (msg, obj, errorlist) {
                    $("#errorlist").empty();
                    $.map(errorlist, function (msg) {
                        $("#errorlist").append("<li>" + msg + "</li>")
            ...
            $("#jobname").formValidator({
                onShow: "请输入职位名称",
                onFocus: "用户名至少2个字符,最多30个字符",
                onCorrect: "该用户名可以注册"
            }).inputValidator({min: 2, max: 30, onError: "你输入的用户名非法,请确认"})
